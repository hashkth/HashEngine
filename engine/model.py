...
DEFAULT_MATERIAL = {
    "ambient": (0.1, 0.1, 0.1), "diffuse": (1.0, 1.0, 1.0),
    "specular": (0.5, 0.5, 0.5), "shininess": 32.0,
    "alpha": 1.0, "alpha_mode": "OPAQUE"
}
...
class GLBModel:
    ...
    def _node_matrix(self, node) -> np.ndarray:
        """Returns the local TRS (or explicit matrix) for a single node."""
    ...
    def _load_meshes(self):
        for node_index, node in enumerate(self.gltf.nodes):
            ...
            for prim_index, prim in enumerate(self.gltf.meshes[node.mesh].primitives):
                ...
                self.meshes[name]    = {"vertices": verts, "normals": normals, "uvs": uvs,
                                        "texture_name": tex_label, "alpha_mode": mat["alpha_mode"]}
